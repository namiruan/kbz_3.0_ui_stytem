...
### AI 활용 가능성 — 시스템 전체 기준
AI가 이 디자인 시스템을 읽고 프로토타입을 생성할 때 일관되게 올바른 결과를 낼 수 있는지 확인한다.

- 컴포넌트 간 `<!-- AI: -->` 주석 패턴이 일관되게 사용되고 있는가 — 어떤 컴포넌트는 상세하고 어떤 컴포넌트는 없는 불균형
- 컴포넌트 선택 기준이 문서화되어 있는가 — 유사 컴포넌트 간 언제 무엇을 써야 하는지 (Badge vs Tag, Button vs IconButton 등)
- 토큰 사용처 주석이 시스템 전체에서 일관된 수준으로 작성되어 있는가
- 시스템 전반의 Do/Don't가 AI가 흔히 저지르는 오용 패턴(하드코딩, 잘못된 토큰 선택, 금지 조합)을 충분히 커버하는가
