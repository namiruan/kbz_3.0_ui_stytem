...
|---|---|
| **접수** | 2026-09-03 |
| **발단** | 게스트·비회원 댓글에서 작성자를 서로 구분해야 함 |
| **상태** | **닫힘** — `tokens/color.md` 아바타 식별색 · `components/atoms/avatar.md` `avatar--c1`~`c8` |

Avatar를 만들 때 "시스템은 사진 없는 사용자의 색을 정해 주지 않는다"를 제약으로 적어 두었고, 그 조건(**아바타 전용 팔레트가 먼저 있을 것**)이 이 요청으로 충족됐다.

**넘긴 것과 다르게 정한 하나** — 미리보기 더미의 대비는 3.72:1이었는데 토큰은 **4.5:1**로 올렸다. 더미는 사람 아이콘, 즉 **그래픽**이라 비텍스트 기준(3:1)만 넘기면 됐지만, 팔레트의 주된 쓰임은 **이니셜**이고 이니셜은 텍스트라 4.5:1이 하한이다. 같은 값을 두 용도로 쓸 수 없어 텍스트 기준에 맞췄다. 사람 마크는 4.5:1이면 여유 있게 통과한다.

**함께 정한 배정 규칙** — `avatarColorIndex(key)`(31진법 누적 해시 → 1~8). `components.js`에 번들되므로 프로토타입이 그대로 부른다. 서버 렌더링이 있으면 같은 식을 서버에서도 써야 첫 화면과 갱신에서 색이 튀지 않는다.

**남는 한계** — 여덟 색이 돌고 동명이인은 색이 겹친다. 색은 **읽기 보조이지 신원이 아니므로** 이름이 항상 함께 있어야 한다. 색을 늘려 해결하지 않는다 — 서로 구분 안 되는 이웃 색이 생겨 오히려 못 읽는다.
