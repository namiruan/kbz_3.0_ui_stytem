...
**세 갈래로 갈렸다** — 색면 + 글(기본) · 이미지 한 장(`--image`) · 이미지 위에 글(`--overlay`). 고르는 기준은 **문구가 어디 있느냐**다: 화면에서 고쳐야 하면 overlay, 디자인이 이미지 한 장으로 완결되면 image, 이미지가 없으면 기본형. overlay의 스크림 60%는 흰 글자가 최악 조건(순백 이미지)에서도 AA를 넘는 **하한**으로 계산해 잡았다(50%면 3.48:1로 모자란다) — 토큰 `--color-surface-scrim-heavy`를 새로 뒀다.
