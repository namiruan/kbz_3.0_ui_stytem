...
정보 테이블과의 차이 — 행의 시각 톤은 정보 테이블에서 가져왔다(가로 구분선만, 줄바꿈 허용). 갈리는 지점은 세 가지다. 정보 테이블은 한 건의 속성을 본문 흐름에 얹지만 게시판은 **하나의 덩어리**라 `md` 이상에서 상자가 된다 — 테두리가 아니라 면과 radius, 가장 얕은 elevation으로. 나머지 둘 — 정보 테이블은 클릭 대상이 아니라 hover를 껐지만 ContentList는 **행 전체가 링크**라 hover가 필수다. 그리고 정보 테이블은 `<table>`이라 컬럼 폭이 고정되지만 ContentList는 `<ul>`이라 폭이 좁아져도 구조가 유지된다.
...
한 줄 배치가 표처럼 읽히지 않게 하는 것은 레이아웃이 아니라 **메타의 처리**다. 분류를 칩으로 만들면 메타 줄에 다른 시각 언어가 섞여 제목과 경쟁하고, 그 순간 컬럼 없는 표가 된다. 메타를 같은 크기·같은 무게의 텍스트로 두면 제목이 위계를 독점한다.
...
메타는 **같은 크기·같은 무게의 텍스트**가 기본이다. 칩을 섞으면 메타가 제목과 시각적으로 경쟁해 "제목이 유일한 목적지"라는 위계가 무너진다. 구분되어야 하는 것은 분류 하나뿐이고, 그건 톤으로 처리한다 — 메타 안에서 분류만 본문 검정이고 나머지는 한 단계 연하다. 브랜드 색은 쓰지 않는다: 목록에서 파란 글자는 hover와 링크를 뜻하는데 분류는 누를 수 없는 값이다.

- ✅ `4대보험 · 2024.03.20 · 조회 1,011` — 같은 무게, 분류만 진하게
- ❌ 분류를 Badge 칩으로 — 메타 줄에 두 가지 시각 언어가 섞인다
...
### 글자 크기는 폭이 아니라 **줄 수**로 정한다

목록에서 폭에 따라 바뀌는 것은 **행 제목 하나**(17 → 15px)다. 나머지는 그대로다 — 메타 13px · 소제목 14px · 열 이름 13px(`sm`에서는 숨는다).

기준은 크기가 아니라 **줄 수**다. 342px에서 제목을 17px로 두면 긴 제목이 두 줄이 되어 행 높이가 들쭉날쭉해지고(실측 92 / 68 / 68…), 높이가 흔들리는 순간 **목록을 훑는 동작이 깨진다.** 15px면 대부분 한 줄에 들어가 행이 고르다(실측 65~66 균일). 메타는 이미 한 줄이라 줄 수가 바뀔 일이 없어 폭을 타지 않는다.

- **게시판 본문 사다리와 같은 규칙, 다른 이유.** 본문은 읽는 글이라 제목이 세 줄이 되면 화면 위를 통째로 쓰고, 목록은 훑는 UI라 행 높이가 흔들리면 훑기가 깨진다. 둘 다 **폭이 줄 수를 바꾸기 때문에** 내린다.
- **크기를 더 잘게 나누지 않는다.** 두 폭(md 이상·sm) 사이에 값이 하나 더 필요해 보이면, 그건 대개 목록이 담은 정보가 많다는 뜻이다 — 메타 슬롯을 줄이는 쪽이 먼저다.
- ⚠️ `sm`에서 소제목(14px)과 행 제목(15px)은 **1px 차이**다. 둘을 가르는 것은 크기가 아니라 **자리**다(머리는 선 위, 아래에 메타 없음). 그래도 얇게 느껴지면 머리를 줄일 것이 아니라 **뺀다**(→ 「머리를 뺄 때」) — 좁은 화면에서 목록이 화면의 전부이면 화면 제목이 이미 그 이름을 말하고 있다.
            ...
            └─ .content-list__body — div. 제목·메타 묶음. flex:1 min-width:0.
                 ...
                 └─ .content-list__meta — div. 부가 정보.
                      **순서 고정: 답변 → 분류 → 작성자 → 작성일 → 조회수.** 최대 5칸까지 열로 정렬된다.
                      ...
                      └─ .content-list__views — span. "조회 1,011". **기본은 텍스트**(단위 라벨 __unit이 뜻을 나른다).
                           아이콘으로 적을 수도 있다 — 그때는 숫자만 낭독되므로 sr-only로 "조회 "를 함께 둔다.
...
- 분류에 Badge(칩)를 쓰지 않는다. 한 줄에 칩이 섞이면 메타가 제목과 시각적으로 경쟁한다.
...
/* 게시판은 페이지에서 **하나의 덩어리**다(소제목 + 열 이름 + 행들이 한 단위로 읽힌다).
   ...
   **테두리는 두지 않는다.** 상자를 만드는 것은 선이 아니라 **면**이다 — 흰 면 + radius +
   ...
   선으로 한 번 더 감싸면 같은 화면에 선이 세 겹이 된다. 데이터 테이블(table-container)은
   가로 스크롤이 생기는 상자라 잘리는 경계를 선으로 명시할 이유가 있지만, 게시판은 접히는
   목록이라 그럴 이유가 없다.

   그림자는 **shadow-sm**이다(`.elevation-base`와 같은 값). 이 시스템의 elevation은
   장식이 아니라 **레이어**를 뜻하고, 게시판은 흐름 안에 있는 base 레이어다 —
   shadow-md는 dropdown, lg 이상은 modal의 값이라 여기에 쓰면 목록이 떠 있는 것으로 읽힌다.
   흰 페이지에서는 아주 옅은 가장자리로, 회색 페이지(surface-subtle) 위에서는
   면 자체로 또렷하게 읽힌다.
...
.content-list__header {
  ...
  background: var(--color-surface-base);
  /* 행 구분선과 **같은 색**이다(--color-border-faint). 상자 안의 모든 가로선은 한 색이다.

     처음엔 --color-border-strong(gray-500)으로 머리선에 위계를 실었다. 근거는
     "머리와 본문은 층위가 다른 구획이다"였고 그 자체는 맞지만, 두 가지가 어긋났다 —
     ① 그 값은 컨테이너에 **테두리가 있던 때** 잡은 것이라, 상자가 선을 버리고
        면과 그림자로 바뀐 뒤에는 이 선만 화면에서 가장 단단한 모서리로 남았다.
     ② 애초에 그 위계를 **선이 말할 필요가 없다.** 머리는 이미 타이포가 말하고 있다 —
        소제목은 17px/600 진한 글자, 열 이름은 13px 회색이고, 머리에는 번호도 링크도 없다.
        선까지 위계를 말하면 같은 말을 두 번 하면서 상자만 딱딱해진다.

     그래서 **선은 리듬만 담당하고 위계는 타이포가 담당한다**로 정리했다.
     gray-500 → gray-300 → gray-100(행 구분선과 통일)을 나란히 렌더해 골랐고,
     통일했을 때도 머리는 그대로 머리로 읽힌다. */
...
/* ── Meta ── */
/* 같은 크기·같은 무게의 텍스트가 기본이다. 칩을 섞으면 메타가 제목과 시각적으로 경쟁한다.
...
@media (max-width: 767px) {
  ...
  /* 행 제목만 한 단계 **내린다**(17 → 15). 목록에서 폭이 바꾸는 것은 크기가 아니라 **줄 수**다 —
     342px에서 17px로 두면 긴 제목이 두 줄이 되어 행 높이가 들쭉날쭉해지고(실측 92 / 68 / 68…),
     높이가 흔들리는 순간 목록을 훑는 동작이 깨진다. 15px면 대부분 한 줄에 들어가 행이 고르다
     (실측 65~66 균일). 게시판 본문 사다리를 내린 이유와 같은 규칙을 목록에 적용한 것이다.
     메타(13px)는 그대로다 — 이미 한 줄이라 줄 수가 바뀔 일이 없다. */
  ...
  /* 머리 크기는 폭을 타지 않는다. 한때 여기에 h4를 다시 적었는데, 그때는 기본값도 h4라
     ...
     상자 이름은 담긴 것이 아니라 상자의 것이라 폭을 타지 않는다.
     ⚠️ 그 결과 sm에서는 소제목(14)과 행 제목(15)이 1px 차이다. 둘을 가르는 것은 크기가 아니라
     **자리**다 — 머리는 선 위에 있고 아래에 메타가 없다. 그래도 얇게 느껴진다면 그건
     머리를 줄이라는 신호가 아니라 **빼라는 신호**다(→ 사용 지침 「머리를 뺄 때」):
     좁은 화면에서 목록이 화면의 전부이면 화면 제목이 이미 그 이름을 말하고 있다. */
...
- 메타를 텍스트로 적으면(`조회 1,011`) 보조 텍스트가 필요 없다 — 단위 라벨이 이미 무엇의 수인지 말한다. **아이콘으로 적는다면** 낭독에는 숫자만 남으므로 `<span class="sr-only">조회 </span>`를 함께 둔다. 아이콘은 이름을 갖지 않는다.
...
> ❌ DON'T — **값**에 칩 쓰기 (행마다 다른 문자열이라 목록이 얼룩이 된다. 칩은 상태 한 열까지)
