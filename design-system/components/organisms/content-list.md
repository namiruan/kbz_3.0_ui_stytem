...
- **고정** — 목록 맨 위에 고정해 두는 항목. 거터의 **핀 아이콘**과 블록을 닫는 경계선으로 표시한다. 라벨도 행 배경도 쓰지 않는다.
...
| 형태 | 아이콘 (`icon-new`, 빨강) | **핀 아이콘 + 블록 경계선** |
| 뜻 | 새로 올라왔다 | 먼저 봐라 |
...
**행 배경을 깔지 않는다.** 주의 톤 면을 거쳐 왔지만, 핀 아이콘이 생긴 뒤로는 같은 말을 두 번 하는 것이 됐다. 색면은 목록에서 가장 비싼 표현이다 — hover(브랜드 틴트)와 자리를 다투고, 읽음 상태와도 겹쳐 보인다. 아이콘과 선으로 이미 전달되는 사실에 쓸 이유가 없다.

**제목 색으로 표시하지도 않는다.** 그 자리는 이미 읽음 여부의 것이다(안 읽음 gray-950 / 읽음 gray-500). 한 속성에 두 의미를 얹으면 "읽은 고정 글"에서 하나가 반드시 진다 — 렌더해 보면 읽은 고정 행이 안 읽은 고정 행과 같은 색이 되어 읽음 규칙이 사라진다. 대비도 모자란다: 주의색 orange-600은 흰 배경에서 **3.92:1**로 AA 미달이고(15px는 large text가 아니다), 통과하는 orange-700(5.35:1)은 제목으로 쓰기엔 탁하다.

**고정 블록은 선으로 닫는다.** 아이콘만으로는 어디까지가 고정인지 경계가 흐리다. 고정이 끝나고 일반 목록이 시작되는 자리에 `--color-border-strong` 선을 둔다 — header 아래 섹션 경계선과 **같은 토큰**이다.

이 선에 **색을 쓰지 않는다.** 주의 색 선을 써 봤지만 목록 안에서 가로로 길게 그어진 유채색은 장식으로 읽힌다 — 행 하나를 칠하는 면과 달리, 선은 색을 쓸 자리가 아니었다. 중립선을 쓰면 목록 안에 새 의미를 만들지 않고 "여기서 구획이 끝난다"는 이미 있는 뜻을 빌리게 된다. 일반 행 구분선(gray-100)과는 명도가 크게 갈려 구분에는 충분하다.

고정 항목끼리는 일반 행 구분선을 그대로 쓴다. 핀이 줄줄이 서 있어 한 덩어리로 읽히므로 그 안에 별도의 선을 둘 이유가 없다.
...
> ⚠️ 고정 항목은 목록 맨 위에 모아 둔다. 중간에 섞이면 닫는 선이 목록을 아무 데서나 자르고, 핀도 날짜 순서를 깬 이유를 설명하지 못한다.
...
레이어 계층: ContentList
  ...
  └─ .content-list — ul. list-style:none.
       └─ .content-list__item — li. **번호 거터 + 본문** 두 열. position:relative (링크 오버레이 기준점).
            ...
            고정 항목에는 content-list__item--pinned를 추가하고(블록을 닫는 경계선),
            거터에 content-list__pin(icon-pin)을 넣는다 — 신규 아이콘과 같은 칸이라 둘을 함께 넣지 않는다.
            뱃지 라벨 슬롯은 두지 않는다 — 표시는 거터 아이콘과 닫는 선뿐이다.
...
/* ── Pinned (optional) ── */
/* 목록 위에 고정해 두는 항목. 표시는 **거터의 핀 아이콘 + 블록을 닫는 선** 둘이다.
   라벨(뱃지)을 두지 않는다 — 운영자가 문서 하나에 내리는 판단은 사실상
   "이건 꼭 봐라" 한 가지뿐이라, 그 하나를 위해 라벨 슬롯을 두면 라벨이
   늘어나기만 한다(서식·인기·NEW…).

   **행 배경을 깔지 않는다.** 주의 톤 면을 써 봤지만, 핀 아이콘이 생긴 뒤로는
   같은 말을 두 번 하는 것이 됐다. 색면은 목록에서 가장 비싼 표현이라
   — hover(브랜드 틴트)와 자리를 다투고, 읽음 상태와도 겹쳐 보인다 —
   아이콘과 선으로 이미 전달되는 사실에 굳이 쓸 이유가 없다.

   제목 색으로 표시하는 방법도 쓰지 않는다. 그 자리는 이미 **읽음 여부**의
   것이다(안 읽음 gray-950 / 읽음 gray-500). 한 속성에 두 의미를 얹으면
   "읽은 고정 글"에서 하나가 반드시 진다 — 실제로 렌더해 보면 읽은 고정 행이
   안 읽은 고정 행과 같은 색이 되어 읽음 규칙이 사라진다.
   주의색은 대비도 모자란다(orange-600 3.92:1 — AA 미달, 15px는 large text가 아니다). */

/* 고정 블록이 끝나는 자리 — 일반 목록과 만나는 경계.
   **중립 선이다.** 주의 색 선을 써 봤지만, 목록 안에서 가로로 길게 그어진 유채색은
   장식으로 읽힌다. 대신 header 아래 섹션 경계선과 **같은 토큰**을 쓴다 —
   그 선이 이미 "여기서 구획이 끝난다"는 뜻으로 쓰이고 있어, 목록 안에 새 의미를
   만들지 않고 있던 뜻을 빌린다. 일반 행 구분선(gray-100)과는 명도가 크게 갈린다.
...
> ✅ DO — 고정은 핀 아이콘 + 블록을 닫는 경계선으로 (색면을 쓰지 않아 hover·읽음 상태와 다투지 않는다)
> `<li class="content-list__item content-list__item--pinned">`
...
> ❌ DON'T — 고정을 제목 색으로 표시하기 (그 자리는 읽음 여부의 것이라 "읽은 고정 글"에서 하나가 진다. 주의색은 대비도 3.92:1로 AA 미달)
> `.content-list__item--pinned .content-list__link { color: var(--color-text-caution); }`
