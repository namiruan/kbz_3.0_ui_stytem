...
- 총 데이터 건수를 표시할 때 toolbar 왼쪽(`table__title-group`) 안, 제목 오른쪽에 둔다. 건수는 "이 테이블에 대한 정보"이므로 액션 영역이 아닌 제목 쪽에 붙인다.
- 제목·액션이 없어도 건수만으로 toolbar를 둘 수 있다. 이때 `table__title-group`에 `table__count`만 넣는다.
...
<div class="table-container">
  <div class="table__toolbar">          ← optional
    <div class="table__title-group">    ← 왼쪽 묶음 (제목 + 총 건수)
      ...
      <span class="table__count" aria-live="polite">총 <b class="table__count-value">120</b>건</span>  ← optional
    </div>
...
- .table__title-group: 제목·건수를 묶는 왼쪽 컨테이너. 건수가 있으면 사용한다. 건수 없이 제목만 둘 땐 .table__title을 toolbar 직계로 둬도 된다(레이아웃 동일).
...
- .table__count: 총 데이터 건수. optional. aria-live="polite" 필수 — 필터 결과로 값이 바뀔 때 읽힘.
...
| 총 건수는 toolbar 왼쪽(`table__title-group`)에 배치 | 총 건수를 `table__toolbar-actions`(우측 액션 영역)에 배치 |
