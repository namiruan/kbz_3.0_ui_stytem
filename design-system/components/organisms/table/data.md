...
(function() {
  ...
  // 전체 선택 (모든 패널의 체크박스에 각각 적용)
  panels.forEach(function(panel) {
    var allCb = panel.querySelector('.table__head .table__cell--check input[type="checkbox"]');
    var rowCbs = panel.querySelectorAll('.table__body .table__cell--check input[type="checkbox"]');
    if (allCb) {
  ...
  function initSort(container) {
    var sortThs = container.querySelectorAll('.table__head-cell--sort');
    ...
    function applySort(th, dir) {
      ...
      btn.querySelector('.tooltip-panel').textContent = dir === 'asc' ? '오름차순' : '내림차순';
    }
    ...
    function clearSort(th) {
      ...
      btn.querySelector('.tooltip-panel').textContent = '오름차순';
    }
    ...
    sortThs.forEach(function(th) {
      ...
      btn.addEventListener('click', function() {
        ...
        if (isDesc) { applySort(th, 'asc'); }
        else if (isAsc) { applySort(th, 'desc'); }
        else { applySort(th, 'asc'); }
      });
    });
  }
  panels.forEach(function(panel) { initSort(panel); });
