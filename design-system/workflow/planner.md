...
### 프로토타입은 로컬 서버로 연다 — `file://`로 열지 않는다

```bash
python3 -m http.server 8000   # 프로토타입 폴더에서
```

`file://`로 열면 브라우저가 **문서마다 다른 출처**로 취급해서 크롬의 일부가 조용히 어긋난다. 바깥 창이 틀(iframe) 안 문서의 주소를 읽지 못하고(`contentWindow.location.href`가 막힌다), 예외를 삼키는 자리라 **콘솔에 아무것도 남지 않는다** — 그래서 "왜 안 되지"의 원인을 찾기 어렵다.

지금은 그 자리에서 **한 번만 경고를 남기고**(`[proto-chrome] … python3 -m http.server 8000`), 막히는 길마다 대체 통로를 둔다 — 그래서 `file://`에서도 이만큼은 된다:

| | `file://` | 로컬 서버 |
|:---|:---|:---|
| 틀 안 시나리오 적용(`?scenario=`) | ✅ (문서가 제 주소를 읽는다) | ✅ |
| 틀 안에서 다른 화면 → 최상위로 나가기 | ✅ (안쪽이 스스로 나온다) | ✅ |
| 폭 인계(`?proto-view`) | ✅ **sessionStorage로만** — 주소·frameElement·referrer가 전부 막힌다 | ✅ |
| 바깥이 틀의 주소를 읽어 판정 | ❌ 막힘(경고 1회) | ✅ |
| 아이콘 스프라이트 | ✅ **규약대로 원격 절대 URL을 쓰면 통과한다**(→ [아이콘 — fetch 주입 패턴](#5-아이콘--fetch-주입-패턴)). 스프라이트를 **로컬 상대경로로 두면 막힌다** — 크롬은 `file://`에 대한 `fetch`를 거부한다 | ✅ 둘 다 |

**서버로 여는 이유는 아이콘이 아니라 마지막 줄에서 두 번째 항목이다** — 바깥이 틀 안을 못 읽으면 크롬이 스스로를 검사할 수 없고, 그 실패가 보이지 않는다. 아이콘은 규약(원격 절대 URL)을 지키면 `file://`에서도 나온다.

---

### 크롬의 생김새 — 꽉 찬 두 열

**크롬은 제품이 아니다.** 전에는 검은 판 위에 둥근 카드(사이드바·화면)를 띄웠는데, 그러면 크롬이 화면보다 먼저 보인다. 지금은 **꽉 찬 두 열**이다 — 왼쪽 300px 목록(흰 바탕 + 오른쪽 구분선), 오른쪽 화면. 구분선 하나로 갈리고 눈은 곧장 화면으로 간다.

오른쪽은 위에서부터 **화면설명 → 툴바(폭 컨트롤) → 무대**다.

- **폭 컨트롤은 화면 바로 위**에 있다. 마크업에서는 사이드바 안에 있지만 **크롬이 DOM에서 옮긴다** — 이미 만들어 둔 프로토타입도 파일을 고치지 않고 새 자리에 놓인다. 목록은 「무엇을 볼지」, 툴바는 「어떻게 볼지」라 축이 다르다.
- **무대는 연한 회색**이고 그 위에 흰 틀이 테두리를 갖고 놓인다. 「어디까지가 화면인가」는 판의 색이 아니라 **틀의 테두리**가 말한다.
- 색·간격·크기는 전부 **시스템 토큰**이다. 크롬이라고 토큰 밖의 값을 쓰지 않는다.

> ⚠️ **틀 안에서는 한 열이다**(`.proto-framed .proto-layout { display: block }`). 안쪽 문서는 사이드바가 `display:none`이라 남는 자식이 `.proto-content` 하나인데, 두 열 격자에서는 **그 하나가 300px 첫 열에 들어간다** — 1282px 틀 안에서 내용이 250px로 짜부라진다(실측).

### 프로토타입 크롬의 컨트롤

사이드바 맨 위에 모여 있다 — **접기 · 화면 목록 · 화면 폭.** 전부 크롬이지 화면의 일부가 아니다.

**셸의 배경은 검정이다.** 화면(틀)이 놓이는 **판**이지 화면의 일부가 아니라서다 — 밝은 회색 위에 흰 화면을 얹으면 어디까지가 화면인지 눈이 매번 다시 찾는다. 판 위의 글자(폭 표시·비교 레이블)는 흰색 계열로 뒤집힌다. 틀 **안**은 그대로 제품 배경이다. 동작은 `components.js`의 `initProtoChrome(document)`이 맡으므로 프로토타입이 따로 구현하지 않는다.
...
**세로도 기기 값이다.** 폭만 맞추면 화면이 창만큼 길어져 **폰인데 세로가 끝없이 긴 화면**이 된다 — "한 화면에 어디까지 들어오는가"가 안 보이므로 접힘·더 보기·하단 버튼 판단이 불가능하다. 창이 그보다 낮으면 바깥 페이지가 세로로 스크롤된다(틀을 줄여 맞추면 버튼이 844를 말하면서 다른 높이를 보여주게 된다).

**틀 안의 조작은 그 틀에만 남는다 — 단 「지금 어느 화면인가」는 바깥으로 전해진다.** 탭·필터·입력 같은 상태는 틀 밖으로 나가지 않는다. 하지만 화면 안 탭이나 스텝으로 **다른 시나리오로 옮겨 가면** 단일 폭에서는 바깥 사이드바의 현재 위치가 따라온다.

전에는 따라오지 않아서, 틀 안은 「B 게시판」인데 사이드바는 「A 게시판」이 켜진 채였다 — **지도가 거짓말을 한다.** 보기 모드 세그먼트를 없앤 것과 같은 종류의 문제이고(→ 아래 절), 그때 내린 결론도 같다: 지도는 따라와야 한다. 게다가 그 상태에서 사이드바의 「A 게시판」을 다시 눌러도 **아무 일도 일어나지 않았다** — 바깥이 아는 주소(`?scenario=a`)가 그대로라 틀을 다시 싣지 않았기 때문이다. 어긋난 것을 되돌릴 수도 없었다는 뜻이다.

- 크롬을 벗어도 **nav 버튼은 틀 안 DOM에 남아 있고**(사이드바만 `display:none`이다) 프로토타입의 `syncNav`가 거기 `is-active`를 계속 옮긴다. 그 클래스 변화를 보고 바깥에 `postMessage`로 이름 하나를 알린다 — **프로토타입 파일은 건드리지 않는다.** 컴포넌트마다 이벤트를 새로 정의할 필요가 없고, 이미 배포된 프로토타입에도 그대로 붙는다.
- 안쪽이 스스로 움직이면 **바깥이 아는 주소를 함께 무효로 만든다.** 그래야 나중에 원래 시나리오 버튼을 눌렀을 때 실제로 다시 실린다.
- **비교 모드에서는 받지 않는다.** 틀이 하나면 "안쪽이 곧 진실"이라 어느 쪽이 진짜인지 정할 필요가 없지만, 셋은 대등해서 그 답이 없다.

> ⚠️ **비교 모드에서 틀 안의 조작은 그 틀에만 적용된다.** 한 틀에서 탭·필터를 만지면 나머지 둘은 그대로라 셋이 서로 다른 상태가 되고, 「같은 화면을 세 폭에서 본다」는 전제가 조용히 깨진다(실측: `lg` 틀에서만 체크 → `[켬, 끔, 끔]`). 바깥이 세 틀의 주소를 다시 싣는 것은 **시나리오를 바꿀 때뿐**이다.
>
> **다시 맞추려면 「비교」를 다시 누른다** — 세 틀을 새로 만들어 같은 출발점으로 돌아온다(실측: `[켬, 끔, 끔]` → `[끔, 끔, 끔]`). 같은 시나리오 버튼을 다시 누르는 것으로는 안 된다 — 주소가 그대로면 다시 싣지 않는다(모드를 오갈 때마다 애먼 새로고침이 일어나지 않게 한 가드다). 비교 모드에서는 사이드바가 접혀 시나리오 버튼이 보이지도 않으므로, 화면에 남아 있는 되돌림 수단은 「비교」 하나다.
>
> **맞추지 않는 이유.** `postMessage`로 안쪽 변화를 바깥에 알려 셋을 묶을 수는 있다. 그러려면 **어느 틀의 조작이 진짜인지**를 정해야 하는데(지금 없는 「마스터 틀」이라는 개념), 셋은 대등하게 놓여 있어서 그런 것이 없다. 그리고 안쪽 상태를 알리려면 **컴포넌트마다 "무엇이 바뀌었다"는 이벤트를 정의**해야 한다 — 프로토타입 크롬을 위해 컴포넌트 API를 늘리는 일이라 꼬리가 몸통을 흔든다. 덧붙여 셋이 다른 상태인 편이 쓸모 있을 때도 있다(`sm`만 필터 시트를 열어 둔 채 나머지 둘과 견주기).
>
> 그래서 **비교는 레이아웃을 재는 장치**로 둔다. 눌러 보며 확인할 것이 있으면 단일 폭(`lg`·`md`·`sm`)에서 한다 — 거기서는 틀이 하나라 어긋날 것이 없다.
...
**틀 안에서 다른 페이지로 넘어가면 페이지 자체가 그리로 간다.** 미리보기는 이 화면이 그 폭에서 어떻게 보이는지를 재는 장치이지 브라우저가 아니다 — 틀 안에서 「목록으로」를 누르면 그 페이지가 제 크롬(사이드바)까지 달고 390px 안에 들어가 제목이 한 글자씩 세로로 쌓인다. 그래서 우리가 띄운 주소(`?proto-frame=1`)가 아닌 문서가 틀에 실리면 **최상위 창을 그 주소로 옮긴다.** 링크의 `target`만 바꾸지 않는 이유는 프로토타입이 JS로 옮기는 경우(`location.href = …`)를 놓치기 때문이다 — 바깥에서 `load`를 보고 판정한다.

**넘어간 화면도 같은 폭에서 열린다.** 보고 있던 모드를 주소에 `?proto-view=sm`으로 남기고, 넘어갈 때 함께 넘긴다 — `sm`에서 목록으로 갔으면 목록도 `sm`으로 뜬다. 폭을 바꿀 때마다 주소가 갱신되므로(`replaceState`) **새로고침해도, 링크를 남에게 보내도** 같은 폭에서 이어진다. 값이 없거나 모르는 값이면 `lg`로 연다. 폭은 세 곳에 실려 있어 하나가 없어도 이어진다 — **틀의 주소**(`?proto-frame=1&proto-view=sm`), 바깥이 iframe에 적어 둔 값, 그리고 **직전 문서(referrer)**. 그래서 바깥 페이지가 옛 번들이어도 틀 안에서 넘어간 화면은 폭을 들고 나온다.

> ⚠️ **판정을 양쪽에서 한다.** 바깥은 iframe의 `load`를 보고 옮기고, **안쪽도** 스스로 확인한다 — "틀 안인데 `?proto-frame=1`이 없으면 최상위로 나간다". 바깥 페이지가 **옛 `components.js`를 캐시**하고 있으면 바깥 규칙이 없어서 넘어간 화면이 제 크롬을 달고 틀 안에 갇히기 때문이다. 안쪽은 방금 새로 불러온 문서라 최신 규칙을 갖는다.
>
> 그래도 화면이 틀 안에 갇힌다면 **자산 캐시**를 의심한다 — 프로토타입은 `components.js?v=20260904a`처럼 고정 쿼리로 부르므로 시스템이 바뀌어도 URL이 그대로다. `?v=` 값을 올리거나 강력 새로고침(⌘⇧R)한다.
...
### 화면설명 — 시나리오가 무엇인지 화면 위에 적는다

**목록은 이름만 말한다.** 「고정 글 없음」이 무엇을 뜻하는지, 그 상태를 어떻게 만드는지, 무엇을 봐야 하는지는 만든 사람 머릿속에만 있었다 — 리뷰하는 사람은 화면만 보고 짐작해야 했다. 그 셋을 **목록 버튼에** 적으면 크롬이 화면 위에 머리글로 그린다.
...
| 속성 | 자리 | 없으면 |
|------|------|--------|
| `data-why` | 제목 아래 한 문단 — **왜 이 화면인가** | 그 줄이 빠진다 |
| `data-steps` | 「이렇게 하면 그 상태가 됩니다」(번호 목록) | 그 열이 빠진다 |
| `data-look` | 「볼 것」(글머리 목록) | 그 열이 빠진다 |
...
> ⚠️ **`scenario-panel`에 적지 않는다.** 크롬은 어느 폭에서든 `.proto-content`를 iframe으로 갈아끼우므로 **패널은 바깥 문서에 없다.** 목록 버튼은 사이드바에 있어 모든 모드에서 그대로 있다.

> **비교 모드에서는 머리글이 숨는다.** 틀이 셋이라 「지금 보고 있는 화면」이 하나가 아니다 — 어느 하나를 골라 적으면 나머지 둘에 대해 거짓말이 된다(밖←안 동기화를 비교 모드에서 받지 않는 것과 같은 판단이다).

### 시나리오는 주소에 남는다 — `#s1` · `#s8`

목록의 **몇 번째인지**가 그대로 주소가 된다. 사이드바에 보이는 번호와 같은 값이라 「8번 보세요」가 링크로 통하고, 새로고침해도 그 자리에 머문다.

- 번호는 **CSS 카운터가 센다** — 마크업에 적지 않는다. 시나리오를 하나 끼워 넣을 때마다 뒤 번호를 손으로 고쳐야 한다면 아무도 안 고치고, 그러면 목록의 번호와 주소의 번호가 어긋난다.
- 그룹 레이블은 세지 않으므로 번호는 **그룹을 가로질러 이어진다**.
- `replaceState`다 — 시나리오를 훑는 것은 방문이 아니다. `pushState`로 쌓으면 뒤로 가기가 목록을 되짚느라 화면 밖으로 나가지 못한다.
...
### 화면 높이에 맞추는 페이지 — `.page--fit`

게시판처럼 **목록이 화면의 주인공**이고 페이지네이션·액션 버튼이 굴리지 않아도 손에 닿아야 하는 화면에서만 `.page`에 `--fit`을 더한다. 페이지가 통째로 굴러가지 않고, 줄어드는 것은 `min-height: 0`을 가진 자식(= `.content-list-container--fit`)뿐이다.

```html
<div class="page page--fit">
  … 페이지 제목 · FilterBar …
  <div data-component class="content-list-container content-list-container--fit"> … </div>
</div>
```

- 목록이 짧으면 상자는 **내용만큼만** 차지한다(화면 끝까지 늘어나 흰 여백을 만들지 않는다).
- `sm`에서는 `.page--fit`이 **스스로 풀린다** — 좁은 화면은 문서 하나로 굴리는 것이 맞다(→ `content-list.md` 「화면 높이에 맞추기」). 프로토타입 쪽에서 따로 할 일은 없다.
- 상자 위에 놓인 것(제목·Breadcrumb·FilterBar)이 화면을 다 먹으면 상자가 화면 밖으로 밀린다. 그건 `--fit`을 쓸 화면이 아니라는 신호다.
- 목록이 아닌 화면(상세·폼)에는 쓰지 않는다. 기본 `.page`는 그대로 흐름대로 굴러간다.
