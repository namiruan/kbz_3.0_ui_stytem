...
- **무대는 어둡다**(`--color-surface-dark`). 밝은 바닥은 흰 화면과의 경계가 테두리 1px에만 걸려 화면 가장자리의 흰 여백이 바닥인지 화면인지 매번 다시 봐야 하고, 크기 표시(`--color-text-inverse-alpha`)가 읽히지 않는다. 어두운 바닥은 「여기는 화면이 아니라 화면을 올려둔 판」이라고 말한다.
- **폭 버튼은 이름 + 숫자다**(데스크톱 1280 · 태블릿 768 · 모바일 390). `lg`는 시스템의 이름이지 폭이 아니라, 무엇을 보고 있는지 알려면 머릿속에서 한 번 더 옮겨야 했다. 폭은 크롬이 이미 아는 값이라 **마크업은 `lg`·`md`·`sm` 그대로** 두고 크롬이 적어 준다.
- **폭·높이 둘 다 끌어서 조절한다** — 오른쪽 모서리(폭) · 아래 모서리(높이) · 오른쪽 아래 모서리(둘 다). 크기는 iframe에 직접 주므로 **안쪽 뷰포트가 실제로 그 크기가 된다** — 높이를 줄이면 `100dvh`·`.page--fit` 같은 규칙이 진짜로 다시 걸린다(화면을 세로로 짧게 만들었을 때 목록이 몇 줄 들어가는지를 재는 것이 이 장치의 쓸모다).
- **기준 크기를 벗어나면 크기 표시의 색이 바뀐다** — 옅은 주의색 칩이 된다. 재는 도구가 「지금 값은 기준이 아니다」를 스스로 말해야, 그 화면을 캡처해 공유했을 때 받는 쪽이 오해하지 않는다. **색만으로 알리지 않는다** — 벗어난 상태에는 글에도 「구간」이 붙는다.
- **끌어서 줄이는 동안에도 지금 어느 구간인지 적힌다**(`980 × 800 px · md 태블릿 구간`). 구간 경계는 `adaptation.md` 그대로다. 버튼이 켜지는 규칙은 그대로 — **켜짐은 「이 폭 그대로」**라는 뜻이라 근처와 같지 않다.
- 색·간격·크기는 전부 **시스템 토큰**이다. 크롬이라고 토큰 밖의 값을 쓰지 않는다.
  - ⚠️ 그래서 기준 화면과 **1~2px 어긋나는 자리가 있다** — 기준의 9px 패딩·1.45 줄간격·18px 번호 폭은 우리 스케일에 없는 값이다. 각각 8px·1.5·20px로 간다. **눈금이 없는 값을 맞추자고 눈금을 깨지 않는다.**
- **접기 손잡이도 툴바에 있다**(폭 세그먼트 왼쪽). 접으면 사이드바가 통째로 사라지므로, 손잡이가 사이드바에 있으면 함께 사라져 되돌릴 자리가 없다.
- **「화면 목록」은 맨 아래다.** 탈출구는 목록을 다 훑고 나서 찾는 것이지 들어가기 전에 먼저 만나는 것이 아니다 — 위에 있으면 시나리오를 고르러 온 눈이 매번 그것을 먼저 지난다.
- **사이드바 자체는 여백을 갖지 않는다.** 이름 블록·목록·화면 목록이 각자 제 여백을 갖는다 — 그래야 이름 블록의 밑선과 화면 목록의 윗선이 **열 전체 폭**으로 그어진다. 사이드바가 좌우 여백을 가지면 그 선들이 안으로 물러나 「덜 그은 선」이 된다.
- **목록 위에 이 프로토타입이 무엇인지 적힌다.** 이름은 문서의 `<title>`에서 온다 — 파일에 또 적게 하면 둘이 어긋난다. 다른 이름을 쓰려면 `.proto-layout[data-proto-title]`(부제는 `data-proto-sub`)로 덮는다.

**목록의 글자는 본문색이다.** 회색으로 두면 전부 「꺼져 있는 것」처럼 보이고 켜진 하나만 읽히는 목록이 된다 — 고르라고 늘어놓은 것인데. 위계는 색이 아니라 **번호(흐림)와 활성 면**이 낸다. 하위 항목은 **들여쓰지도, 레일을 긋지도 않는다**: 그룹 레이블 아래로 이어지는 번호가 곧 「이 묶음」이라 선은 같은 말을 두 번 하는 것이고, 활성 면과 자리를 다툰다.

> ⚠️ **좌우 구분선은 `.proto-content`가 긋는다.** 사이드바에 `border-right`를 걸면 sticky + `max-height: 100dvh` 때문에 화면보다 긴 페이지에서 선이 중간에 끊긴다(실측: 격자 행 1081px인데 사이드바는 950px에서 멈췄다). 내용은 언제나 행 전체 높이다.

> ⚠️ **바깥에서만 뜻이 있는 규칙은 `.proto-framed`에서 되돌린다.** 안쪽 문서도 같은 크롬 CSS를 싣기 때문이다. 지금까지 두 번 걸렸다 — 격자(아래)와 **세로 구분선**(`.proto-content`의 `border-left`가 안쪽에도 걸려 화면 왼쪽 끝에 1px 줄이 그어졌다). 안쪽에는 사이드바가 없으니 **가를 것도 없다.**
...
**넘어간 화면도 같은 폭에서 열린다.** 보고 있던 모드를 주소에 `?proto-view=sm`으로 남기고, 넘어갈 때 함께 넘긴다 — `sm`에서 목록으로 갔으면 목록도 `sm`으로 뜬다. 폭을 바꿀 때마다 주소가 갱신되므로(`replaceState`) **새로고침해도** 같은 폭에서 이어진다. 값이 없거나 모르는 값이면 **기기가 정한다** — 창이 900px 미만이면 틀을 만들지 않고(기기 폭 그대로), 그보다 넓으면 `lg`다. 폭은 세 곳에 실려 있어 하나가 없어도 이어진다 — **틀의 주소**(`?proto-frame=1&proto-view=sm`), 바깥이 iframe에 적어 둔 값, 그리고 **직전 문서(referrer)**. 그래서 바깥 페이지가 옛 번들이어도 틀 안에서 넘어간 화면은 폭을 들고 나온다.
...
**좁은 기기에서는 틀을 만들지 않는다.** 창이 900px 미만이면(크롬이 「자리가 없다」고 판단하는 그 폭) 첫 화면은 틀 없이 **기기 폭 그대로**다. 폰에서 `lg`로 열면 390 안에 1280 틀이 들어와 가로로 스크롤되는 데스크톱 화면이 뜬다 — **기기에서 보려고 연 링크인데 기기를 안 보여 준다.** 틀은 *다른* 폭을 재는 도구이지, 지금 그 폭으로 보는 수단이 아니다. 이때 세그먼트는 아무것도 가리키지 않으므로 툴바가 **「이 기기 390」**으로 지금 폭을 적는다(창을 줄이면 따라간다). 데스크톱 레이아웃을 보고 싶으면 폭 버튼을 누르면 된다.

### 링크 복사 — 이 시나리오를 다른 기기에서

툴바 오른쪽 끝의 **「링크 복사」**가 지금 보고 있는 시나리오의 주소를 복사한다. 목록의 번호가 곧 `#s`번호라, 그 링크를 폰에서 열면 **같은 시나리오가 그 자리에서 열린다.**

| 주소에 남는 것 | 복사될 때 |
|---------------|-----------|
| `#s8` (지금 시나리오) | **남긴다** — 이것이 「무엇을」이다 |
| `?proto-view=sm` (보던 폭) | **덜어 낸다** — 폭은 **받는 기기가 정한다.** 그대로 보내면 폰 안에 390 틀이 또 생긴다 |
| `?proto-frame=1` · `?scenario=` (크롬의 내부 값) | 덜어 낸다 |
| 프로토타입 제 쿼리 (`?docId=42` 같은) | **남긴다** — 「어떻게 볼까」가 아니라 「무엇을」의 일부다 |

- **버튼은 결과를 글자로 말한다** — `복사됨`. 색만 바꾸면 색을 못 보는 사람에게는 아무 일도 일어나지 않는다.
- ⚠️ **`file://`이나 `localhost`에서 복사하면 `복사됨 · 이 컴퓨터에서만`이라고 적는다.** 복사는 되지만 그 주소는 다른 기기에서 열리지 않는다 — 말없이 보내면 폰 앞에서야 안다. 같은 공유기의 `192.168.x.x`는 폰에서 실제로 열리므로 경고하지 않는다.
- 복사는 **두 벌**이다. `navigator.clipboard`는 보안 컨텍스트(https·localhost)에서만 있는데, 폰에서 보려고 `http://192.168.x.x`로 띄운 순간이 정확히 그 경우다. 그때는 `execCommand`로 복사하고, 둘 다 안 되면 주소를 띄워 손으로 복사하게 한다 — **복사가 안 됐는데 됐다고 말하지 않는다.**
...
> **비교 모드에서도 머리글이 보인다.** 비교는 서로 다른 화면 셋이 아니라 **한 시나리오를 세 폭으로** 보는 것이라(세 틀에 같은 주소가 들어간다) 「어느 시나리오인가」의 답은 하나다. 머리글은 폭을 말한 적이 없다. — 밖←안 동기화를 비교 모드에서 받지 않는 것과는 다른 문제다: 그건 「어느 틀의 조작이 진짜인가」라 답이 없지만, 시나리오는 셋 다 같다.
...
**목록에는 열이 둘 있다 — 번호 열과 글자 열.** 번호는 왼쪽 20px 열에 곧게 남고, **묶음 이름이든 항목 이름이든 글자는 전부 그 오른쪽**에서 시작한다. 이름표를 왼쪽 끝에 두었더니 묶음 이름이 번호와 같은 자리에서 시작해, 번호가 이름 앞에 붙은 것처럼 보였다 — 줄이 「묶음 이름」인지 「번호 붙은 항목」인지 왼쪽 끝만 봐서는 갈리지 않는다.

| 신호 | 1단계 | 2단계 |
|------|-------|-------|
| 번호 | 20px (한 열 고정) | **같은 열** |
| 이름표 | 48px · 본문색 + heading weight, **위에 구분선** | 60px · 흐린 색 + body weight |
| 항목 이름 | 48px | **60px** (한 칸 더) |

- **이름표는 제 항목들의 이름과 같은 선에 선다** — 그 열의 머리글이 되는 자리다. 2단계 묶음은 이름표와 항목이 함께 12px 더 들어가, 1단계 직속 항목과 갈린다.
- **미는 것은 이름뿐이고 번호는 제 열에 남는다.** 줄 전체를 밀면 번호 열이 둘로 갈려 세로로 훑기 어려워진다 — 번호를 넣으면서 그룹 레일을 걷어낸 것과 같은 판단이다.
- **구분선이 구역의 끝을 말한다.** 여백만으로 나누면 묶음이 여럿일 때 어디서 끝났는지가 흐려진다 — 사이가 벌어진 것과 구역이 바뀐 것은 다른 말이다.
- **단계는 세어서 적는다 — 마크업에 적지 않는다.** 크롬이 목록을 순서대로 훑어 `data-nav-level`을 붙인다(번호를 카운터가 세는 것과 같은 이유 — 어긋날 수 있는 값은 두지 않는다). 형제 선택자로는 「이 이름표부터 다음 이름표까지」를 가둘 수 없다.
- **묶음은 다음 이름표에서 닫힌다.** 2단계 묶음을 벗어나 1단계 직속으로 돌아가려면 다음 이름표를 쓰거나 `proto-nav-sub` 없는 **최상위 항목**을 둔다 — 그냥 이어 적으면 앞 묶음에 딸린다.
