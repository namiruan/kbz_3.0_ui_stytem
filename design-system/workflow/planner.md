...
**나무는 세 가지가 함께 만든다** — 하나만으로는 단계가 구분되지 않았다.

| 신호 | 1단계 | 2단계 |
|------|-------|-------|
| 이름표 | 본문색 + heading weight, **위에 구분선** | 흐린 색 + body weight, 40px 들여씀 |
| 항목 이름 | 48px | **64px** (한 칸 더 들여씀) |
| 번호 | 20px 열 고정 | **같은 열** |

- **구분선이 구역의 끝을 말한다.** 여백만으로 나누면 묶음이 여럿일 때 어디서 끝났는지가 흐려진다 — 사이가 벌어진 것과 구역이 바뀐 것은 다른 말이다.
- **미는 것은 이름뿐이고 번호는 제 열에 남는다.** 줄 전체를 밀면 번호 열이 둘로 갈려 세로로 훑기 어려워진다 — 번호를 넣으면서 그룹 레일을 걷어낸 것과 같은 판단이다. 2단계 이름표는 1단계 **항목 이름과 같은 시작선**(40px)에 서고, 그 아래 항목이 한 칸 더 들어가 「이 이름 아래 이 항목들」이 된다.
