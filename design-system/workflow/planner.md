...
**넘어간 화면도 같은 폭에서 열린다.** 보고 있던 모드를 주소에 `?proto-view=sm`으로 남기고, 넘어갈 때 함께 넘긴다 — `sm`에서 목록으로 갔으면 목록도 `sm`으로 뜬다. 폭을 바꿀 때마다 주소가 갱신되므로(`replaceState`) **새로고침해도, 링크를 남에게 보내도** 같은 폭에서 이어진다. 값이 없거나 모르는 값이면 `lg`로 연다.
