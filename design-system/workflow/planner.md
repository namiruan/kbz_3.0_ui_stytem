...
3. **HTML 생성 — 3단계로 순서대로 진행**
   ...
   **Phase 3 — JS 구현**  
   ...
   - **같은 상태(검색결과없음·오류 등)가 정적 패널과 JS 양쪽에 존재하면 셸·empty 액션 버튼·pagination 가시성까지 동일** — 데이터 상태는 별도 패널로 복제하지 말고 tbody만 교체한다(→ 조회 페이지 상태 원칙). 0건이면 pagination 숨김
...
> **조회 페이지 상태 = 셸 1벌 + tbody 교체 (중복 금지)** — `데이터있음`·`검색결과없음`·`로딩`·`오류`는 서로 다른 페이지가 아니라 **같은 페이지의 tbody 상태**다. 같은 상태를 두 곳(정적 패널 + JS)에서 따로 만들면 셸·액션이 어긋난다(필터바·정렬·pagination·「필터 초기화」 버튼 불일치). 방지 규칙:
> ① 필터바·정렬 헤더·툴바·pagination(= **셸**)은 **한 번만** 작성하고, 상태별로는 **tbody(데이터 영역)만 교체**한다. 데이터 상태를 별도 full-page 패널로 복제하지 않는다.
> ② 빈·로딩·오류 tbody는 **단일 출처**(JS 렌더 함수 1개 또는 `<template>` 1개)에서 만들어, 정적 스냅샷과 실시간 필터 결과가 **같은 마크업을 복제**하게 한다 — 「필터 초기화」 등 내부 액션까지 포함.
> ③ 결과 **0건이면 pagination을 숨긴다** (표시할 데이터가 없으면 페이지 네비가 의미 없음).
