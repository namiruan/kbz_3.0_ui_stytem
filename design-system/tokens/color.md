...
| `action` | 브랜드·파란 계열 | `--color-action-brand-hover`, `--color-action-brand-selected`, `--color-action-brand-overlay`, `--color-action-brand-selected-dark` |
| `action` | 보조 브랜드·청록 계열 | `--color-action-info-hover`, `--color-action-info-filtered`, `--color-action-info-subtle` |
| `action` | 위험 액션 | `--color-action-error-hover` |
