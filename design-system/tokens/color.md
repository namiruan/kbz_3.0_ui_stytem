...
밝기 스케일(50–950)로 구성된다. 숫자가 클수록 어둡다. 팔레트별 원시값이며 컴포넌트에서 직접 참조하지 않는다.
...
| `background` | UI 요소 단색 배경 — 버튼·칩 등 인터랙티브 컴포넌트 전용. 레이아웃 면적(surface)과 달리 채도 있는 고정 색이 필요할 때 사용 | `--color-background-brand`<br>`--color-background-neutral`<br>`--color-background-error` |
