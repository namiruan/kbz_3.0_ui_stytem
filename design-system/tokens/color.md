...
> 흰 글자 → `--color-surface-scrim-heavy`(검정 60%, 최악 4.81:1) · 검은 글자 → `--color-surface-scrim-heavy-inverse`(흰 60%, 최악 6.47:1). 50%(`--color-action-neutral-overlay`)는 흰 글자에서 3.48:1로 **모자란다** — 그건 아이콘·컨트롤을 얹는 값이지 글을 얹는 값이 아니다
