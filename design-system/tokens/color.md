...
| 그룹 | 사용처 | 토큰 |
|------|--------|------|
| `surface` | 중립 배경 | `--color-surface-base`<br>`--color-surface-subtle`<br>`--color-surface-neutral`<br>`--color-surface-disabled`<br>`--color-surface-disabled-strong`<br>`--color-surface-dark`<br>`--color-surface-dim`<br>`--color-surface-scrim`<br>`--color-surface-scrim-heavy`<br>`--color-surface-scrim-heavy-inverse` |
...
> ✅ DO — 이미지 위 글자에는 극성에 맞는 60% 스크림을 깐다
> 흰 글자 → `--color-surface-scrim-heavy`(검정 **60%**, 최악 4.81:1) · 검은 글자 → `--color-surface-scrim-heavy-inverse`(흰 **50%**, 최악 4.63:1).
>
> **두 값이 다른 것은 실수가 아니라 감마다.** 흰 스크림은 어두운 픽셀을 훨씬 빨리 끌어올려 하한이 49.2%로, 검정 스크림의 58%보다 8.8%p 낮다. 같은 숫자로 맞춰 두면 밝은 갈래만 필요 없는 10%p를 더 쓰고, 그 대가를 「밝은 일러스트가 뿌옇게 뜬다」로 치른다. 외우기 쉬운 것보다 **각자의 하한에 맞는 것**이 낫다.
>
> ⚠️ 검정 쪽은 **낮출 수 없다.** 58%가 하한이라 60%는 이미 2%p 위이고, 낮춰 봐야 얻는 것도 없다 — 어두운 사진 위에서 60% → 45%는 바닥 휘도를 0.0129 → 0.0156으로 옮길 뿐이라 **눈에 보이지 않는다**(실측). 검정 스크림의 값어치는 밝은 이미지에서만 나타나고, 비용은 그때만 든다.
>
> 50%짜리 `--color-action-neutral-overlay`를 글 밑에 쓰지 않는다 — 그건 아이콘·컨트롤을 얹는 값이다(흰 글자 기준 3.48:1로 모자란다).

> ❌ DON'T — 스크림 위의 글자를 반투명으로 두기
> `color: var(--color-text-inverse-alpha)` — 스크림이 이미 최악 조건에 딱 맞춰져 있어서, 글자까지 반투명이면 그만큼 대비가 깎인다(실측: 흰 65%면 4.81 → **3.05:1**). 스크림 위에서는 **불투명 색만** 쓰고, 제목과 설명의 위계는 크기와 굵기가 낸다
