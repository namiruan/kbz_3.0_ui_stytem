...
**본문은 크기 대신 줄간격을 좁힌다.** `sm`에서 `.text-board-body`의 `line-height`가 `prose`(1.625) → `reading`(1.5)이 된다. 글자는 17px 그대로다.

줄간격은 글자 크기가 아니라 **줄 길이**에 맞춘다 — 1.625는 데스크톱의 긴 줄에서 눈이 다음 줄 첫 글자를 놓치지 않게 하는 값인데, 390px에서는 한 줄이 24자밖에 안 된다. 그만한 leading이 필요 없고 오히려 문단이 성글어져 한 덩어리로 안 읽힌다.

| 안 | 본문 높이(390px · 3문단 573자) | 줄당 글자 |
|:---|:---|:---|
| 17px · 1.625 (이전) | 497px | 24자 |
| **17px · 1.5 (지금)** | **459px (−8%)** | 24자 |
| 15px · 1.625 | 414px (−17%) | 28자 |
| 15px · 1.5 | 383px (−23%) | 28자 |

긴 B2B 문서에서 세로를 줄일 이유는 실제로 있다. 다만 그 값을 **글자 크기가 아니라 줄간격에서** 낸다 — 15px로 내리면 더 줄지만 그건 **읽는 크기**를 깎는 것이라 세로를 아끼려고 읽기를 파는 셈이다. 줄당 24자는 이미 한글 본문의 편안한 범위이고, 크기를 줄여 얻는 것은 2~4자뿐이다.
