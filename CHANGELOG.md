...
- **Avatar 문서 미리보기의 `image` 더미를 사람마다 다른 색으로 바꿨다** — 여덟 개가 전부 같은 파란 그림이라 "사진이 들어가는 자리"라는 것만 보이고 목록에서 사람이 갈린다는 느낌이 없었다. 색조마다 연한 면 + **같은 색조를 진하게 한** 사람 아이콘으로 바꿨다.
  - **아이콘 명도는 색조별로 따로 풀었다.** 여덟 색조에 같은 명도(L 38)를 쓰면 대비가 **3.13~6.92:1**로 널뛴다 — 초록·청록은 흐리고 남보라는 새까맣다. 색조마다 4.5:1에 닿는 **가장 밝은** 명도를 찾아 전부 **4.6~5.8:1**로 모았다. 필요 이상으로 어둡게 만들지 않으면서 여덟 개가 고르게 읽힌다.
  - ⚠️ **이건 미리보기 더미이지 시스템 기능이 아니다.** 이름을 해시해 색을 배정하는 방식(Slack·Google)은 목록에서 사람을 구분해 주지만 **아바타 전용 팔레트**가 먼저 있어야 한다 — 지금 팔레트의 red·green·orange는 상태를 뜻해서 사람에게 쓰면 뜻이 겹친다. 팔레트를 정의하기 전에는 시스템이 색을 배정하지 않는다는 것을 제약에 적었다. 사진이 없으면 이니셜, 그것도 없으면 로고다. avatar.md v0.2.0 → v0.2.1
