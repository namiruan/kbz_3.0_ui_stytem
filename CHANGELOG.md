...
### Added
- **ContentList 문의 게시판 변형** — 작성자(`__author`)와 답변(`__answers`) 슬롯 추가. 장애신고·묻고답하기처럼 **사용자가 쓰고 담당자가 답하는** 목록은 메타가 다르다: 자료실은 `분류·작성일·조회수`지만 문의 게시판은 `분류·작성자·작성일·답변`이고, 조회수는 뺀다(대부분 비밀글이라 판단 근거가 못 된다).
  ...
  - 4·5칸일 때의 기본 트랙 폭은 CSS가 슬롯 수를 세어 정한다(`:where(:has(.content-list__meta > :nth-child(4)))` — `:where()`로 명시도를 0으로 둬서 호스트의 `--content-list-meta-cols` 한 줄이 이긴다). 호스트가 변수를 잊어도 라벨이 다른 열 위에 서는 조용한 오류가 나지 않는다.
  - 1280px·390px 렌더로 검증: 3·4·5칸 모두 라벨과 값의 열 좌표가 정확히 일치하고(예: 4칸 872/984/1080/1176), empty와 본목록의 열 위치도 그대로 같다. 390px 가로 스크롤 없음, 인라인 메타는 `이콘 · 김지현 · 2026.08.31 · 답변 대기`.
