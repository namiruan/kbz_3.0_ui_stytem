...
### Added
- **토큰 `--color-text-caution-muted`(orange-700 #a8531f) 추가** — 주의 계열에서 **본문 대비 기준을 맞춰야 할 때** 쓰는 억제 톤. `--color-text-caution`(orange-600)은 흰 배경 3.92:1로 AA 본문 기준(4.5:1)에 미달인데, 그동안 그 사실이 ContentList 고정 제목의 ⚠️ 주석에만 적혀 있고 **대안 값은 토큰이 아니라 primitive(`--color-orange-700`)로만 언급돼 있었다.** 브랜드·info가 이미 갖고 있는 `-muted` 패턴(blue-600→800, cyan-600→700)을 주의 계열에도 맞춰 채웠다. 5.35:1로 AA 통과. tokens/color.md v1.8.0 → v1.9.0
- **ContentList 댓글 수를 주의 계열 억제 톤으로** — 브랜드 파랑에서 바꿨다. 목록에서 색을 쓰는 자리는 고정(제목·핀)·신규(아이콘)·댓글 수 셋뿐이고, 셋 다 "이 행을 먼저 봐라"는 **주목** 신호다. 같은 계열로 묶되 서로 갈리게 했다: 고정 `caution`(orange-600) · 댓글 수 `caution-muted`(orange-700) · 신규 `fill-error`(red-600). 명도와 형태(제목 semibold 17px / 대괄호 숫자 13px / 16px 아이콘)로 구분된다.
  - orange-600을 그대로 쓰지 않은 이유 두 가지 — 고정 제목과 **정확히 같은 색**이 되고, AA 미달(3.92:1)이다. 네 후보(orange-600 · orange-700 · red-600 · brand)를 고정·신규가 같이 있는 목록에 나란히 렌더해 골랐다.
  - 참고: `icon-new`는 주황이 아니라 **빨강**(`--color-fill-error` #bb2e2e)이다. 화면에서 주황으로 보이던 것은 고정 핀(`--color-fill-caution` orange-500)이었다. content-list.md v0.41.0 → v0.42.0
