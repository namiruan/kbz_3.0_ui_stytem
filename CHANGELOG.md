...
### Changed
- ContentList: 분류(`__cat`)를 **칩**으로. Badge 컴포넌트를 쓰되 `badge--brand badge--line badge--pill` — 플래그(`rect + 칠해진 tint`)와 **두 축**(모양·채움)이 모두 다르다. 색만 다르면 둘 다 상태 표시로 읽히지만, 모양과 채움이 함께 다르면 종류가 다르게 읽힌다(신규=아이콘 / 플래그=뱃지에 쓴 것과 같은 원리). 기존 "분류에 Badge를 쓰지 않는다" 규칙은 **인라인 한 줄 메타**를 전제로 쓴 것이었다 — 열이 생긴 데스크톱과 메타가 별도 줄로 내려가는 `sm`에서는 칩이 제목과 경쟁하지 않고, 특히 `sm`은 열 이름이 사라져 분류를 짚어줄 것이 칩밖에 없다. 메타에서 형태를 갖는 것은 분류 하나로 제한하는 규칙으로 대체했다(날짜·조회수는 같은 무게 텍스트). 곁들여 고친 것 둘: 열 간격을 `padding` → `margin`으로 옮겼다(padding이면 그 여백이 **칩 안쪽**으로 들어가 칩이 부풀고 글자가 밀린다 — 라벨과 값 양쪽에 같은 margin을 줘 열 정렬은 유지, 측정 856.8/972.8/1068.8 일치), 그리고 칩이 열 전체로 늘어나지 않게 `justify-self: start`. 분류가 칩이면 메타의 가운뎃점을 전부 빼고 간격(md)으로 나눈다 — 좁은 화면에서 메타가 접힐 때 다음 줄이 "· 조회 89"로 시작하는 것을 막고 폭도 줄인다. content-list.md v0.22.0 → v0.23.0 (MINOR)
