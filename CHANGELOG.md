...
- **FileUpload 좌우 배치 variant 추가** (`file-upload--split`) — 안내·추가하기 버튼을 왼쪽(**왼쪽 정렬**), 파일 목록을 오른쪽에 둔다. 기본(위아래) 배치는 **파일이 늘수록 드롭 영역이 아래로 길어져 폼 안에서 다음 필드를 밀어낸다**(첨부 아래 보안코드가 있는 글쓰기 화면). 좌우로 가르면 드롭 영역의 높이가 목록 높이에 묶여 폼의 세로 길이가 안정된다 — 카드 2장 기준 드롭 영역 높이 **290px → 196px**.
  ...
  - **파일이 없으면 한 칸으로 돌아간다**(`:not(:has(.file-upload__grid))`) — 오른쪽에 빈 칸이 남으면 드롭 영역의 절반이 이유 없이 비어 보인다.
  - **`sm`에서는 위아래로 돌아가고 정렬도 함께 되돌린다** — 왼쪽 정렬은 좁은 칸에서 문장과 버튼의 시작점을 맞추려던 것이라, 한 칸으로 돌아오면 그 이유가 사라진다. 남겨두면 같은 화면에서 기본 배치와 이 variant가 이유 없이 달라 보인다. 두 칸으로 가르지 않는 이유는 카드가 한 장도 온전히 들어가지 않기 때문이다(카드 최소 폭 150px + 좌우 inset). 1000·390px 렌더로 확인했다. file-upload.md v0.4.0 → v0.5.1
