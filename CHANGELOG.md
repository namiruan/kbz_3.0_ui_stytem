...
### Changed
- **ContentList 머리의 글씨를 20 → 17px, 밑선을 gray-500 → gray-300으로 내렸다.** 상자 이름이 목록이 담은 글보다 크고, 밑선이 상자를 가로지르는 검은 자처럼 보였다.
  ...
  - **밑선 — 기준이 바뀐 걸 놓쳤다.** 처음 `--color-border-strong`(gray-500)으로 잡을 때는 컨테이너에 테두리가 있어서 그 선들 사이에서 머리선이 한 단계 위로 읽혔다. 그런데 같은 사이클에 **상자가 선을 버리고 면과 그림자로 바뀌면서**(md 이상 radius + shadow) 이 선만 남아 화면에서 가장 단단한 모서리가 됐다. **기준이 바뀌면 그 기준으로 잡았던 값도 다시 재야 한다** — 값 하나를 고친 게 아니라 값이 기대던 맥락이 사라진 경우다.
  - strong·default·subtle을 나란히 렌더해 골랐다. `subtle`(gray-200)까지 내리면 행 구분선(gray-100)과 구분되지 않아 **머리 구획 자체가 사라진다** — 행 구분선보다 두 단계 위인 `default`(gray-300)가 바닥이다. content-list.md v0.47.0 → v0.48.0
