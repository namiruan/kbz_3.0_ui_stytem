...
### Changed
- ContentList: 필터 행이 있으면 `sm`에서 **행마다의 분류(`__cat`)를 숨긴다.** 같은 정보를 목록 위에서 한 번, 행마다 한 번 두 번 말할 이유가 없고 그 중복이 좁은 메타 줄을 접히게 만든다. **필터 행이 있을 때만** 숨긴다 — 없는 목록에서 숨기면 분류를 알 방법이 사라진다. 숨긴 분류 바로 뒤의 가운뎃점도 함께 지웠다: 구분자는 `> :not(:first-child)::before`로 붙는데 `display:none`이어도 DOM에는 남아 날짜가 여전히 first-child가 아니라, 메타 줄이 "· 2024.03.20"처럼 가운뎃점으로 시작했다(실측). CSS로 "보이는 것 중 첫 번째"를 고를 수 없어 인접 선택자로 짚는다. content-list.md v0.23.0 → v0.24.0 (MINOR)
