...
### Added
- `scripts/sync_icons.py`: 대상 **페이지를 이름으로 지정**할 수 있게 했다(`FIGMA_ICON_PAGE`). 지금까지는 노드 `0:1` — **파일의 첫 페이지**를 하드코딩하고 있었다. 아이콘 파일은 첫 페이지가 곧 ICON 페이지라 드러나지 않던 제약인데, 페이지가 여럿인 디자인 시스템 라이브러리 파일을 가리키는 순간 문제가 된다. 비우면 첫 페이지를 쓰므로 기존 동작은 그대로다. 못 찾으면 그 파일의 페이지 이름 목록을 찍고 종료한다.
