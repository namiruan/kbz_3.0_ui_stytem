...
### Added
- **ContentList가 `md` 이상에서 상자가 된다** — 테두리 + radius. 게시판은 페이지에서 **하나의 덩어리**인데(소제목 + 열 이름 + 행들이 한 단위로 읽힌다) 넓은 화면에서는 목록 좌우에 페이지 여백이 남아, 경계가 없으면 목록이 어디서 시작하고 끝나는지 페이지 배경과 갈리지 않았다. 값은 **`table-container`와 같다**(`organisms/table/index.md` — border-subtle 1px · radius-md 8px · overflow hidden): 데이터 테이블과 게시판이 같은 무게의 상자로 보여야 둘을 가르는 것이 상자가 아니라 내용이 된다.
