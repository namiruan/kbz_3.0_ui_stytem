...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  /* ─── 하이트 스케일 (Y축 보조선) ─── */
  .height-vis { position: relative; margin: var(--space-8) 0 var(--space-24); font-family: var(--font-family-mono); font-size: var(--font-size-label-xs); }
  .height-y-label {
    position: absolute; left: 0; width: 36px;
    text-align: right; color: var(--color-text-subtle);
  ...
  .height-bar:hover { transform: translateY(-2px); }
</style>
</head>
<body>
  ...
  (function() {
    ...
    function renderPage(slug) {
      ...
      bodyEl.querySelectorAll('.scale-placeholder').forEach(function(el) {
        ...
        if (type === 'height') {
          ...
          var BAR_GAP = 20;
          var AXIS_W = 44;
          var maxPx = entries[entries.length - 1].px;
          var visH = maxPx + PAD_TOP;

          var vis = document.createElement('div');
          vis.className = 'height-vis';
          vis.style.height = visH + 'px';

          // 보조선 + px 레이블
          entries.forEach(function(e) {
            var topPos = visH - e.px;
            var lbl = document.createElement('span');
            lbl.className = 'height-y-label';
            lbl.style.top = topPos + 'px';
            lbl.textContent = e.px + 'px';
            var guide = document.createElement('div');
            guide.className = 'height-guide';
            guide.style.top = topPos + 'px';
            vis.appendChild(lbl);
            vis.appendChild(guide);
          });
          ...
          entries.forEach(function(e, i) {
            ...
            bar.setAttribute('data-token-value', e.key);
            bar.style.height = e.px + 'px';
            bar.style.width = BAR_W + 'px';
            bar.style.left = (AXIS_W + i * (BAR_W + BAR_GAP)) + 'px';
            vis.appendChild(bar);
          });
    ...
    document.addEventListener('mouseover', function(e) {
      ...
      var code = e.target && e.target.closest
        ? (e.target.closest('code[data-token-value]') || e.target.closest('.palette-chip[data-token-value]') || e.target.closest('.scale-unit[data-token-value]') || e.target.closest('.height-bar[data-token-value]'))
        : null;
      if (!code) {
