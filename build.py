...
<header class="topbar">
  ...
  <span class="version-pill">v0.34.0</span>
...
.proto-nav {
  display: flex; flex-direction: column; counter-reset: proto-sc;
  /* 항목이 맞붙지 않는다 — 활성 면이 둥근 모서리를 가지므로 붙어 있으면 그 모서리가
     이웃과 닿아 뭉개져 보인다. 좌우 여백은 그 면이 사이드바 벽에 붙지 않게 한다. */
  gap: var(--space-2); padding: 0 var(--space-8);
}
.proto-nav-btn {
  /* **baseline이다.** 번호(12)와 이름(14)은 크기가 다르므로 가운데로 맞추면 번호가
     살짝 떠 보인다. 글줄의 바닥을 맞춰야 한 줄로 읽힌다. */
  display: flex; align-items: baseline; gap: var(--space-8); position: relative;
...
.proto-nav-btn::before {
  ...
  /* 두 자리(10·11)가 와도 이름의 시작선이 밀리지 않을 만큼 잡는다 */
  flex: none; min-width: var(--space-20);
...
/* ⚠️ 여기서 `padding-left: 0`으로 되돌리려다 **버튼 제 좌우 여백까지 죽였다**
   (실측: `6px 12px 6px 0px` — 하위 항목만 왼쪽 여백이 사라졌다). 되돌릴 것이 없으면
   규칙을 두지 않는 것이 맞다 — 기본값을 덮어쓰는 규칙은 그 자체가 새 기본값이 된다. */
...
/* ── 링크 복사 ── */
/* 주소에 `#s`번호를 남긴 것은 **「8번 보세요」가 링크로 통하게** 하려던 것인데, 정작 그
   주소를 가져갈 길이 주소창을 손으로 읽는 것뿐이었다 — 폰으로 열어 보려면 더 그렇다.
   툴바 **오른쪽 끝**에 선다: 왼쪽 컨트롤은 「어떻게 볼까」(접기·폭)이고 이것은
   「이걸 가져가기」라 축이 다르다. 한 줄에 두되 자리로 갈라 놓는다. */
.proto-share {
  margin-left: auto;
  padding: var(--space-inset-squish-sm);
  border: var(--stroke-sm) var(--stroke-solid) var(--color-border-subtle);
  border-radius: var(--radius-sm);
  font-family: var(--font-family-base); font-size: var(--font-size-sm);
  color: var(--color-text-subtle); background: var(--color-surface-base);
  cursor: pointer; white-space: nowrap;
}
.proto-share:hover { background: var(--color-surface-subtle); color: var(--color-text-body); }
/* **됐다는 말은 글자로 한다.** 색만 바꾸면 색을 못 보는 사람에게는 아무 일도 일어나지 않는다 */
.proto-share.is-done { color: var(--color-text-success); border-color: var(--color-text-success); }
/* 이 컴퓨터에서만 열리는 주소였을 때 — 복사는 됐으니 막지 않고, 다만 그 사실을 말한다.
   「다른 기기에서 보려고」 누른 버튼이 말없이 쓸모없는 주소를 주면 폰 앞에서야 안다. */
.proto-share.is-local {
  color: var(--color-text-caution-muted); border-color: var(--color-text-caution-muted);
  background: var(--color-surface-caution-subtle);
}
/* 틀 없이 기기 폭 그대로 볼 때 — 세그먼트가 아무것도 가리키지 않으므로 지금 폭을 적는다.
   폭을 말하지 않는 화면은 재는 도구가 아니다(크기 표시와 같은 자리의 판단이다). */
.proto-device { font-size: var(--font-size-sm); color: var(--color-text-subtle); white-space: nowrap; }
.proto-device b {
  color: var(--color-text-body); font-weight: var(--font-weight-heading);
...
/* 아래 — 세로. 폭과 달리 틀은 **위가 고정**이라(무대가 세로로 쌓는다) 높이는 이동거리
   그대로 변한다. 손잡이 모양도 눕혀서 어느 축인지 손대기 전에 알 수 있게 한다. */
.proto-frame-handle--v {
  top: auto; left: 0; right: 0; bottom: calc(var(--space-8) * -1);
  width: auto; height: var(--space-16); cursor: ns-resize;
}
.proto-frame-handle--v::before { width: var(--space-48); height: var(--space-4); }

/* 모서리 — 둘 다. 「모서리를 끌어 조절」이라 적어 놓고 정작 모서리가 없었다.
   DOM에서 맨 뒤에 둬 두 모서리 손잡이의 끝자락보다 위에 온다. */
.proto-frame-handle--corner {
  top: auto; left: auto;
  right: calc(var(--space-8) * -1); bottom: calc(var(--space-8) * -1);
  width: var(--space-16); height: var(--space-16); cursor: nwse-resize;
}
.proto-frame-handle--corner::before {
  width: var(--space-8); height: var(--space-8); border-radius: var(--radius-xs);
}
