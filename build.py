...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .height-arrow-head { font-size: 8px; line-height: 1; flex-shrink: 0; }
  .height-arrow-line { flex: 1; width: 1px; background: var(--color-border-brand); }
  .height-arrow-val { font-size: var(--font-size-label-xs); color: var(--color-text-brand); white-space: nowrap; margin-top: 2px; flex-shrink: 0; }
  .height-label { color: var(--color-text-subtle); text-align: center; max-width: 80px; word-break: break-all; }
</style>
</head>
<body>
      ...
      bodyEl.querySelectorAll('.scale-placeholder').forEach(function(el) {
        ...
        if (type === 'height') {
          ...
          entries.forEach(function(e) {
            ...
            var arrowTop = document.createElement('span');
            arrowTop.className = 'height-arrow-head';
            arrowTop.textContent = '▲';

            var arrowLine = document.createElement('div');
            arrowLine.className = 'height-arrow-line';

            var arrowBot = document.createElement('span');
            arrowBot.className = 'height-arrow-head';
            arrowBot.textContent = '▼';

            var arrowVal = document.createElement('span');
            arrowVal.className = 'height-arrow-val';
            arrowVal.textContent = e.px + 'px';

            arrow.appendChild(arrowTop);
            arrow.appendChild(arrowLine);
            arrow.appendChild(arrowBot);
          ...
          // 패딩 박스: 공간이 콘텐츠를 감싸며 박스 크기 자체가 커짐
          var SCALE = 2;
          var strip = document.createElement('div');
          ...
          entries.forEach(function(e) {
            ...
            box.style.padding = Math.max(1, Math.round(e.px * SCALE / 4)) + 'px ' + Math.max(1, Math.round(e.px * SCALE)) + 'px';
