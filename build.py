...
<style>
  ...
  * { box-sizing: border-box; margin: 0; padding: 0; }
  ...
  .zindex-iso-container { position: absolute; overflow: visible; }
  .zindex-iso-top { border-radius: var(--radius-xs) var(--radius-xs) 0 0; transform: skewY(-22deg); transform-origin: left top; cursor: default; transition: filter var(--duration-fast) ease; }
  .zindex-iso-top:hover { filter: brightness(1.06); }
  .zindex-iso-front { border-radius: 0 0 var(--radius-xs) var(--radius-xs); }
  ...
  (function() {
    ...
    function renderPage(slug) {
      ...
      bodyEl.querySelectorAll('.zindex-placeholder').forEach(function(el) {
        ...
        // 낮은 z → 밝은 색, 높은 z → 진한 색 (상단면 / 앞면)
        var topColors   = ['#e8eaed','#d8dee6','#c6cedb','#b6c0d0','#a6b2c5','#96a4b8','#8a94a8'];
        var frontColors = ['#bec2c8','#b0b8c4','#a0aab8','#909cac','#808ea0','#748090','#6c7888'];
        var topH    = 14;   // 상단면 높이
        var frontH  = 6;    // 앞면(두께) 높이
        var layerW  = 180;
        var legendW = 44;
        var gap     = 10;
        var scale   = 0.8;
        ...
        var sceneH = (maxVal - minVal) * scale + topH + frontH;
        ...
        order.forEach(function(key, i) {
          ...
          var val        = vals[i];
          var containerTop = (maxVal - val) * scale;
          var centerY    = containerTop + topH / 2;

          // 레이어 컨테이너 (상단면 + 앞면)
          var container = document.createElement('div');
          container.className = 'zindex-iso-container';
          container.style.cssText = 'top:' + containerTop + 'px;left:' + (legendW + gap) + 'px;width:' + layerW + 'px;';

          // 상단면 (skewY로 기울어진 플레이트 윗면)
          var topFace = document.createElement('div');
          topFace.className = 'zindex-iso-top';
          topFace.setAttribute('data-token-value', key);
          topFace.style.cssText = 'width:' + layerW + 'px;height:' + topH + 'px;background:' + topColors[i] + ';';
          container.appendChild(topFace);

          // 앞면 (두께를 나타내는 직사각형)
          var frontFace = document.createElement('div');
          frontFace.className = 'zindex-iso-front';
          frontFace.style.cssText = 'width:' + layerW + 'px;height:' + frontH + 'px;background:' + frontColors[i] + ';';
          ...
          nameEl.style.cssText = 'top:' + (containerTop + topH / 2 - 7) + 'px;left:' + (legendW + gap + layerW + gap) + 'px;';
